...
from IPython.display import display, DisplayObject, Markdown, Latex, Code, HTML
import requests
import re
import copy
import pandas
...
def do_render_code(file, lang="c++", show=None, line_numbers=True, trim_ends=False, demangle=None):
    ...
    if isinstance(show, str):
        if lang == "c++":
                # Matches C++ source definitions like 'int baseline_int(' or 'void baseline_int ('
                show = (rf"\b{re.escape(show)}\s*\(", r"^\s*\}")
    ...
    if show is not None and len(show) == 2:
        if isinstance(show[0], str):  # regexes
            ...
            # Fallback if start wasn't found, so it doesn't dump the whole file blindly
            if not found_start:
                raise ValueError(f"Could not find function matching start pattern: {show[0]}")
            if not found_end:
                # If .cfi_endproc isn't found, just grab a reasonable chunk or go to end
...
def ask_gemma(
    prompt: str, temperature: float = 0.0, seed: int = 42
) -> DisplayObject:
    """Sends a prompt to the local Gemma model on 'eevee' with consistency settings,

    and renders the response as Markdown in a Jupyter Notebook.
    """
    url = "http://eevee:11434/api/generate"

    # Ollama uses an 'options' dictionary for generation parameters
    payload = {
        "model": "gemma4:e4b",
        "prompt": prompt,
        "stream": False,
        "options": {
            "temperature": temperature,  # 0.0 = most deterministic/consistent
            "seed": seed,  # Fixed seed = reproducible outputs
        },
    }
    ...
    try:
        ...
        # Extract the generated text from the Ollama response JSON
        result_text = response.json().get("response", "")

        # Render and display as Markdown in Jupyter
        md_output = Markdown(result_text)
        display(md_output)
